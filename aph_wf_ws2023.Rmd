...
```{r}
# Counting number of Whitefly Adults data on bottom leaf
...
Summed_WF_Aph<- ddply(dpr_data2023, c("Product","TRT","Block", "Leaf_Location", "DPR_Label", "App", "DAT"), summarise,
...
Summed_WF_Aph.Avg<- ddply(Summed_WF_Aph, c("Product","TRT", "Leaf_Location", "DPR_Label", "DAT"), summarise,
                     ...
                     Aph.Tot.SumSD   = sd(Aph.Tot.Sum, na.rm=TRUE),
                     Aph.Tot.SumSE   = Aph.Tot.SumSD / sqrt(N) ) 
...
# Summed DPR with only product

#Summed DPR
SummedDPR_Product<- ddply(dpr_data2023, c("TRT","Product","Block",  "DPR_Label"), summarise,
                    WF.Nym.Sum = sum(WF.Nym, na.rm=TRUE),
                    WF.Ad.Sum = sum(WF.Ad, na.rm=TRUE),
                    Aph.Tot.Sum = sum(Aph.Tot, na.rm=TRUE))
...
#Summed DPR
SummedDPR<- ddply(dpr_data2023, c("TRT","DPR_Label"), summarise,
                    WF.Nym.Sum = sum(WF.Nym, na.rm=TRUE),
                    WF.Ad.Sum = sum(WF.Ad, na.rm=TRUE),
                    Aph.Tot.Sum = sum(Aph.Tot, na.rm=TRUE))

SummedDPR
# Average of total with DPR label

SummedDPR.Avg.proudct.1 <-SummedDPR_Product %>%
  dplyr::group_by(DPR_Label)%>%
  dplyr::summarise(
    N = sum(!is.na(WF.Nym.Sum)),
    WF.Nym.SumAvg = mean(WF.Nym.Sum, na.rm=TRUE),
    WF.Ad.SumAvg = mean(WF.Ad.Sum, na.rm=TRUE),
...
SummedDPR.App2.bottom <- SummedDPR.App2 %>%
  filter (Leaf_Location %in% c("Bottom"))


SummedDPR.App2.bottom

total_wfnymph_app2_bottom<-lm(WF.Nym.Sum~DPR_Label+Block,data = SummedDPR.App2.bottom)
Anova(total_wfnymph_app2_bottom)

LS_total_wfnymph_app2 <- emmeans(total_wfnymph_app2_bottom, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_total_wfnymph_app2 $emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_total_wfnymph_app2_bottom;CLD_total_wfnymph_app2_bottom
...
# With total number of whitefly nymphs 

```{r}
# Total Whitefly nymphs
WF.Nym_total_lm<-lm(sqrt(WF.Nym.Sum)  ~ DPR_Label + Block , data = SummedDPR_Product)

Anova(WF.Nym_total_lm)

LS_WF.Nym_total_lm <- emmeans(WF.Nym_total_lm, pairwise ~ DPR_Label,adjust="FDR",type="response") 

cld(LS_WF.Nym_total_lm $emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_WF.Nym_total_lm;CLD_WF.Nym_total_lm

CLD_WF.Nym_total_lm %>%
  arrange((DPR_Label))

#model comparison with or without Block 
WF.Nym_total_lm.1<-lm(sqrt(WF.Nym.Sum)  ~ DPR_Label  , data = SummedDPR_Product)

anova_result <- anova(WF.Nym_total_lm, WF.Nym_total_lm.1)
print(anova_result)

```

```{r}
# Total Whitefly nymph

XX<-as.data.frame(CLD_WF.Nym_total_lm[order(CLD_WF.Nym_total_lm),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$WF.Nym.SumAvg<-SummedDPR.Avg.proudct.1$WF.Nym.SumAvg

WFNymChart_dpr2023_total <- Summed_WF_Aph.Avg %>%
  ggplot(aes(fill=forcats::fct_rev(DAT), y=WfNym.SumAvg, x=DPR_Label)) +
  ...
  ggtitle("Total") +
  ylim (0,20)+
  geom_text(data=XX ,aes(x=DPR_Label,y= (WF.Nym.SumAvg)*1.1,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)
 
 
WFNymChart_dpr2023_total

ggsave(path = "Figure", file="WFNymChart_dpr2023_total.jpg", width=10, height=6, dpi=600)

```

## Total whitefly nymphs top leaf 

```{r}
total_wfnymph_top <- Summed_WF_Aph%>%
  filter (Leaf_Location == "Top")

#total_wfnymph_top
#model
T_avg_nymph_top<-lm(sqrt(WfNym.Sum)~DPR_Label,data = total_wfnymph_top )
Anova(T_avg_nymph_top)

#model check
check_model(T_avg_nymph_top)

LS_nymph_top_total <- emmeans(T_avg_nymph_top, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_nymph_top_total$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_nymph_top_total;CLD_nymph_top_total
...
total_wfnymph_bottom <- Summed_WF_Aph%>%
  filter (Leaf_Location == "Bottom")

#total_wfnymph_top
#model
T_avg_nymph_bottom<-lm(sqrt(WfNym.Sum)~DPR_Label,data = total_wfnymph_bottom )
Anova(T_avg_nymph_bottom)

check_model(T_avg_nymph_bottom) # model checking

LS_nymph_bottom_total <- emmeans(T_avg_nymph_bottom, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_nymph_bottom_total$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_nymph_bottom_total;CLD_nymph_bottom_total


CLD_nymph_bottom_total %>%
  arrange((DPR_Label))
```

#Total average whitefly nymphs top leaf

```{r}
# #Total average whitefly nymphs top leaf
XX<-as.data.frame(CLD_nymph_top_total[order(CLD_nymph_top_total),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$WF.Nym.SumAvg<-SummedDPR.Avg.proudct.1$WF.Nym.SumAvg

WFNymChart_dpr2023_total_top <- Summed_WF_Aph.Avg %>%
  filter (Leaf_Location == "Top")%>%
  ggplot(aes(fill=forcats::fct_rev(DAT), y=WfNym.SumAvg, x=DPR_Label)) +
  geom_bar(position="stack", stat="identity", color="#333333", size=0.1) +
  scale_fill_brewer(palette="Spectral") +
  ...
  theme(axis.text.x=element_text(color="#000000", size=12),
        axis.text.y=element_text(color="#000000", size=12)) +
  ylab("Avg. Whitefly nymph (per leaf)") +
  xlab ("Insecticides and spray coverage")+
  labs(fill="DAT") + 
  ggtitle("Total: Top Leaf") +
  
  geom_text(data=XX ,aes(x=DPR_Label,y= (WF.Nym.SumAvg)*1.1,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)
 
 
WFNymChart_dpr2023_total_top

ggsave(path = "Figure", file="WFNymChart_dpr2023_total_top.jpg", width=10, height=6, dpi=600)
```

## Total average whitefly nymphs bottom location 

```{r}
##Total average whitefly nymphs bottom leaf
XX<-as.data.frame(CLD_nymph_bottom_total[order(CLD_nymph_bottom_total),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$WF.Nym.SumAvg<-SummedDPR.Avg.proudct.1$WF.Nym.SumAvg

WFNymChart_dpr2023_total_bottom <- Summed_WF_Aph.Avg %>%
  filter (Leaf_Location == "Bottom")%>%
  ggplot(aes(fill=forcats::fct_rev(DAT), y=WfNym.SumAvg, x=DPR_Label)) +
  geom_bar(position="stack", stat="identity", color="#333333", size=0.1) +
  scale_fill_brewer(palette="Spectral") +
  ...
  xlab ("Insecticides and spray coverage")+
  labs(fill="DAT") + 
  ggtitle("Total: Bottom Leaf") +
  ylim (0, 15)+ 
  geom_text(data=XX ,aes(x=DPR_Label,y=WF.Nym.SumAvg,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)
 
 
WFNymChart_dpr2023_total_bottom

ggsave(path = "Figure", file="WFNymChart_dpr2023_total_bottom.jpg", width=10, height=6, dpi=600)
```
...
aphid_total_lm<-lm(sqrt(Aph.Tot.Sum)~DPR_Label + Block,data = SummedDPR_Product)
Anova(aphid_total_lm)

check_model(aphid_total_lm) # model checking

LS_aphid_total_lm <- emmeans(aphid_total_lm, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_aphid_total_lm$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_aphid_total_lm;CLD_aphid_total_lm
...
XX<-as.data.frame(CLD_aphid_total_lm[order(CLD_aphid_total_lm),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$Aph.Tot.SumAvg<-SummedDPR.Avg.proudct.1$Aph.Tot.SumAvg

AphidChart_dpr2023_total <- Summed_WF_Aph.Avg %>%
  ...
  ylim (0,800)+
  ggtitle("Total")+ 
  geom_text(data=XX ,aes(x=DPR_Label,y= (Aph.Tot.SumAvg)*1.1,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)

 
AphidChart_dpr2023_total

ggsave(path = "Figure", file="AphidChart_dpr2023_total.jpg", width=10, height=6, dpi=600)
 
  
```

## Total aphid by leaf locations

```{r}
# Total aphid top leaf
total_aphid_top <- Summed_WF_Aph%>%
  filter (Leaf_Location == "Top")

aphid_total_top<-lm(sqrt(Aph.Tot.Sum)~DPR_Label,data = total_aphid_top)
Anova(aphid_total_top)

#model check
check_model(aphid_total_top)

LS_aphid_total_top <- emmeans(aphid_total_top, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_aphid_total_top$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_aphid_total_top;CLD_aphid_total_top

CLD_aphid_total_top %>%
  arrange((DPR_Label))


# Total aphid bottom leaf
total_aphid_bottom <- Summed_WF_Aph%>%
  filter (Leaf_Location == "Bottom")

aphid_total_bottom<-lm(sqrt(Aph.Tot.Sum)~DPR_Label,data = total_aphid_bottom)
Anova(aphid_total_bottom)

#model check
check_model(aphid_total_bottom) 

LS_aphid_total_bottom <- emmeans(aphid_total_bottom, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_aphid_total_bottom$emmeans,alpha   = 0.05,Letters = letters,adjust="FDR")->CLD_aphid_total_bottom;CLD_aphid_total_bottom

CLD_aphid_total_bottom %>%
  arrange((DPR_Label))
```
...
```{r}
XX<-as.data.frame(CLD_aphid_total_top[order(CLD_aphid_total_top),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$Aph.Tot.SumAvg<-SummedDPR.Avg.proudct.1$Aph.Tot.SumAvg


AphidChart_dpr2023_total_top <- Summed_WF_Aph.Avg %>%
  filter (Leaf_Location == "Top")%>%
  ggplot(aes(fill=forcats::fct_rev(DAT), y=Aph.Tot.SumAvg, x=DPR_Label)) +
  geom_bar(position="stack", stat="identity", color="#333333", size=0.1) +
  scale_fill_brewer(palette="Spectral") +
...
AphidChart_dpr2023_total_top

ggsave(path = "Figure", file="AphidChart_dpr2023_total_top.jpg", width=10, height=6, dpi=600)
```

## Total aphid bottom location 

```{r}
# Total aphid bottom location

XX<-as.data.frame(CLD_aphid_total_bottom[order(CLD_aphid_total_bottom),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
XX$Aph.Tot.SumAvg<-SummedDPR.Avg.proudct.1$Aph.Tot.SumAvg

AphidChart_dpr2023_total_bottom <- Summed_WF_Aph.Avg %>%
  filter (Leaf_Location == "Bottom")%>%
  ggplot(aes(fill=forcats::fct_rev(DAT), y=Aph.Tot.SumAvg, x=DPR_Label)) +
  geom_bar(position="stack", stat="identity", color="#333333", size=0.1) +
  scale_fill_brewer(palette="Spectral") +
...
AphidChart_dpr2023_total_bottom

ggsave(path = "Figure", file="AphidChart_dpr2023_total_bottom.jpg", width=10, height=6, dpi=600)
```
...
#model check
check_model(WF.Nym_Summed_app1) 
...
#model check
check_model(WF.Adult_Summed_app1) 
...
# Model check
check_model(WF.Nym_Summed_top_app1) 
...
WFNymChart_dpr2023_top_app1 <- SummedDPR.App1.Avg %>%
  ...
  coord_cartesian(ylim=c(0,(max(XX$WF.Nym.SumAvg*3))))+
  geom_text(data=XX ,aes(x=DPR_Label,y=(WF.Nym.SumAvg)*1.2,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)
...
# Model check
check_model(WF.Nym_Summed_bottom_app1) 
...
WFNymChart_dpr2023_bottom_app1 <- SummedDPR.App1.Avg %>%
  ...
  ylim(0,2) 
WFNymChart_dpr2023_bottom_app1 + geom_text(data=XX ,aes(x=DPR_Label,y=WF.Nym.SumAvg,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)
 
 


ggsave(path = "Figure", file="WFNymChart_dpr2023_bottom_app1.jpg", width=10, height=6, dpi=600)
...
SummedDPR_Top_App1 <-SummedDPR.App1%>%
  dplyr::filter (Leaf_Location == "Top")

WF.Ad_Summed_top_app1<-lm(sqrt(WF.Ad.Sum)~DPR_Label,data = SummedDPR_Top_App1)
#WF.Nym_Summed <- glm.nb(round(WF.Nym.Sum,0)~DPR_Label+Block,data=SummedDPR)
Anova(WF.Ad_Summed_top_app1)

# Model check
check_model(WF.Ad_Summed_top_app1) 
...
WFAdultChart_dpr2023_app1 + geom_text(data=XX ,aes(x=DPR_Label,y= (WF.Ad.SumAvg)*1.2,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)

ggsave(path = "Figure", file="WFAdultChart_dpr2023_app1.jpg", width=10, height=6, dpi=600)
...
# Model check
check_model(Aph.Tot_Summed_top_app1)
...
TotalAphidChart_dpr2023_top_app1 <- SummedDPR.App1.Avg %>%
  ...
  geom_text(data=XX ,aes(x=DPR_Label,y= (Aph.Tot.SumAvg)*1.2,label=group,fill=NULL), vjust = -1, hjust = .5, na.rm = TRUE)

TotalAphidChart_dpr2023_top_app1
ggsave(path = "Figure", file="TotalAphidChart_dpr2023_top_app1.jpg", width=10, height=6, dpi=600)
...
# Model check
check_model(Aph.Tot_Summed_bottom_app1)
...
ggsave(path = "Figure", file="TotalAphidChart_dpr2023_bottom_app1.jpg", width=10, height=6, dpi=600)
...
# Model check
check_model(WF.Nym_Summed_top_app2)
...
ggsave(path = "Figure", file="WFNymChart_dpr2023_top_app2.jpg", width=10, height=6, dpi=600)
```
...
wfnymph_bottom_app2<-lm (sqrt(WF.Nym.Sum) ~DPR_Label ,data = SummedWF_bottom_app2) 
Anova(wfnymph_bottom_app2)

# Model check
#check_model(wfnymph_bottom_app2)

LS_wfnymph_bottom_app2 <- emmeans(wfnymph_bottom_app2, pairwise ~ DPR_Label,adjust="FDR",type="response") #;LS_WF.Nym_Summed

cld(LS_wfnymph_bottom_app2$emmeans,alpha = 0.05, Letters = letters,adjust="FDR")->CLD_wfnymph_bottom_app2;CLD_wfnymph_bottom_app2
...
```{r}
#Whitefly nymphs average App2: Bottom leaf 
...
XX<-as.data.frame(CLD_wfnymph_bottom_app2[order(CLD_wfnymph_bottom_app2),])
XX$group<-gsub("[[:space:]]", "", XX$.group)
...
ggsave(path = "Figure", file="WFNymChart_dpr2023_bottom_app2.jpg", width=10, height=6, dpi=600)
```
...
SummedDPR.App2_top <-  SummedDPR.App2 %>%
  dplyr:: filter (Leaf_Location == "Top")
  
WF.Ad_Summed_top_app2<-lm(sqrt(WF.Ad.Sum)~DPR_Label,data = SummedDPR.App2_top)
#WF.Nym_Summed <- glm.nb(round(WF.Nym.Sum,0)~DPR_Label+Block,data=SummedDPR)
Anova(WF.Ad_Summed_top_app2)

# Model check
check_model(WF.Ad_Summed_top_app2)
...
ggsave(path = "Figure", file="WFAdultChart_dpr2023_app2.jpg", width=10, height=6, dpi=600)
```
...
TotalAphidChart_dpr2023_top_app2 <- SummedDPR.App2.Avg %>%
  ...
  ggtitle("App. 2: Top leaf")+
  ylim (0, 500)+
  geom_text(data=XX ,aes(x=DPR_Label,y= (Aph.Tot.SumAvg)*1.1,label=group,fill=NULL), vjust = -1, hjust = .5)
 
 
TotalAphidChart_dpr2023_top_app2

ggsave(path = "Figure", file="TotalAphidChart_dpr2023_top_app2.jpg", width=10, height=6, dpi=600)
```
...
Aph.Tot_Summed_bottom_app2<-lm(sqrt(Aph.Tot.Sum)~DPR_Label,data = Summed_Aph_Tot_bottom_app2)
#WF.Nym_Summed <- glm.nb(round(WF.Nym.Sum,0)~DPR_Label+Block,data=SummedDPR)
Anova(Aph.Tot_Summed_bottom_app2)

# Model check
check_model(Aph.Tot_Summed_bottom_app2)
...
## Calculating difference between top and bottom location by average summed aphid 


```{r}
# Calculate the mean coverage difference between 5th Leaf and 9th Leaf
library(dplyr)
avg_aphid_diff <- SummedDPR.Avg %>%
  dplyr::group_by(DPR_Label) %>%
  dplyr::summarise(avgerge_aphid_diff = mean(Aph.Tot.SumAvg[Leaf_Location == "Top"]/Aph.Tot.SumAvg[Leaf_Location == "Bottom"], na.rm = TRUE)) %>%
  ungroup()
...
```{r}
# Percent difference whitefly nymph

library(dplyr)
avg_wfnymph_diff <- SummedDPR.Avg %>%
  dplyr::group_by(DPR_Label) %>%
  dplyr::summarise(avgerge_whiteflynymph_diff = mean(WF.Nym.SumAvg[Leaf_Location == "Top"]/WF.Nym.SumAvg[Leaf_Location == "Bottom"], na.rm = TRUE)) %>%
  ungroup()
